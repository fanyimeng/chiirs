...
def kdeplot(data, kdecolor, kdeorder, kdeax, text):
    ...
    #           linestyle='--', color=vlinecolor)
    # ax.text(np.power(10, xx)[np.argmax(kde(xx))], 0.52,
    #         '$\\leftarrow 10^{%.2f}$' % (np.log10(np.power(10, xx)[np.argmax(kde(xx))])),
    #         size=8, rotation=90.,
    #         ha="center", va="bottom", color = kdecolor)
    ax.text(np.nanmean(data), 0.5,
            '$\\leftarrow \\rm mean=10^{%.2f}$' % (np.log10(np.nanmean(data))),
            size=9, rotation=90.,
            ha="center", va="bottom", color = kdecolor)
    ax.text(np.nanmedian(data), 0.5,
            '$\\leftarrow \\rm med=10^{%.2f}$' % (np.log10(np.nanmedian(data))),
            size=9, rotation=90.,
            ha="center", va="bottom", color = kdecolor)
...
ax.set_xlabel('${\\rm EM}$ [${\\rm pc\\,cm^{-6}}$]')
...
kde_t1 = kdeplot(data=pltdata, kdeorder=0,
                 kdecolor=(0.8, 0.5, 0.5, 0.8), kdeax=ax,
...
#                 a=ella * 0.05,
#                 b=ellb * 0.05)
# kde_t2 = kdeplot(data=pltdata, kdeorder=2,
#                  kdecolor=(0.5, 0.5, 1.0, 0.8), kdeax=ax,
#                  text='$M_\\mathrm{Hii}$ (UCHii)')


ax.set_xlim([np.nanmin(pltdata)/3, np.nanmax(pltdata)*3])
