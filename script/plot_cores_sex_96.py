...
def plotcores(ax, coresa, coresb, linewidth=1):
    ...
    xlim = ax.get_xlim()
    ylim = ax.get_ylim()
    for i in range(len(coresa['cenx'])):
        within = True
        within = coresa['cenx'][i] > xlim[0] and within
        within = coresa['cenx'][i] < xlim[1] and within
        within = coresa['ceny'][i] > ylim[0] and within
        within = coresa['ceny'][i] < ylim[1] and within
        if within:
            num = num + 1
            if coresa['cenf_096'][i] > coresa['cenf_006'][i] * (96/6.)**0.3:
                bnum = bnum + 1
    ...
    for i in range(len(coresb['cenx'])):
        within = True
        within = coresb['cenx'][i] > xlim[0] and within
        within = coresb['cenx'][i] < xlim[1] and within
        within = coresb['ceny'][i] > ylim[0] and within
        within = coresb['ceny'][i] < ylim[1] and within
        if within:
...
coresa = pd.read_csv('cores_096_flxcompare.csv')
coresb = pd.read_csv('cores_006_em.csv')
...
plt.savefig('../plot/overall_cores_096.pdf', bbox_inches='tight')
plt.clf()
