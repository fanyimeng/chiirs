...
def kdeplot(data, kdecolor, kdeorder, kdeax, text):
    ...
    #           linestyle='--', color=vlinecolor)
    ax.text(np.nanmean(data), 0.6,
            '$\\leftarrow \\rm mean=%.1f$' % (np.nanmean(data)),
            size=7, rotation=90.,
            ha="center", va="bottom", color = kdecolor)
    ax.text(np.nanmedian(data), 0.67,
            '$\\leftarrow \\rm med=%.1f$' % (np.nanmedian(data)),
            size=7, rotation=90.,
            ha="center", va="bottom", color = kdecolor)
