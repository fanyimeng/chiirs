...
def em(coresa='cores_006_coll.csv', nu=6., te=1e4, outputcsv='cores_006_coll.csv'):
    cores = pd.read_csv(coresa)
    a = cores['ella'] * 0.05
    b = cores['ellb'] * 0.05
    emarray = 5.36e3 * nu**0.1 * te**0.35 * \
        cores['flx_iso']  / a / b
    narray = np.sqrt(emarray / (np.sqrt(a * b) / 50. * 2.))
    cores['em'] = emarray
    cores['n'] = narray
    cores.to_csv(outputcsv)
    return 0


em(coresa='cores_006_coll.csv', outputcsv='cores_006_em.csv')
# collidelist('cores_096_coll.csv')
